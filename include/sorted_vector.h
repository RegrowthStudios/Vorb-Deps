...
namespace codeproject {
    ...
    public:
        ...
        explicit sorted_vector(const Pr& pred = Pr(), const A& al = A())
            ...
            static_assert(CHECK::EqualExists<K>::value, "Key type must implement equality operator.");
        }
#if (_MSC_VER >= 1300)
        template<class It>
        sorted_vector(It first, It beyond,
            ...
            static_assert(CHECK::EqualExists<K>::value, "Key type must implement equality operator."); stable_sort();
        }
#else
        ...
        Pairib_ insert(const value_type& x) {
            if (bNoDuplicates){
                ...
                if (p == end()){
                    ...
                    while (p != end() && !key_compare_(x, *p)) {
                        if (x == *p) return Pairib_(p, false);
                        ++p;
                    }
                    return Pairib_(InsertImpl_(p, x), true);
                }
